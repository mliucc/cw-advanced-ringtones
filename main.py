...
noon_class = '午自习'      #午餐铃或午自习铃对应课程(如上一项为1则在''内填写该活动名称；如上一项为0则在''内填写下面一个活动的活动名称，且上一节课、该课间与下一节课在同一节点内)
vol = 75    #铃声音量，1～100的整数
#配置铃声区域
        ...
        if self.method.is_get_notification():
            if self.cw_contexts['Notification']['state'] == 2:    #判定是否放学
                ...
                if self.cw_contexts['Notification']['state'] == 0:
                    ...
                    logger.info('插件cw-ring-personality检测到其他通知，不进行打铃')
